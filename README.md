...
4. Discord-verified submissions publish immediately. You can still hide a review in **Table Editor → reviews** by changing `status` to `rejected`.
...
- A database trigger derives the Supabase UUID and Discord identity from the authenticated JWT, marks the identity verified, and publishes the review as `approved`.
- Browser roles receive no update or delete permission, so reviewers cannot moderate records.
- A verified client may submit multiple reviews and each one publishes immediately.
